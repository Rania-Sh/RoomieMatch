...
def get_matches(user_id: int, gender: str = None):
    conn = get_conn()
    cursor = conn.cursor()
    ...
    # ← פילטר לפי עיר + מגדר אם נבחר
    if gender and gender != "any":
        cursor.execute(
            'SELECT * FROM "user" WHERE id != %s AND city = %s AND gender = %s',
            (user_id, current[3], gender)
        )
    else:
        cursor.execute(
            'SELECT * FROM "user" WHERE id != %s AND city = %s',
            (user_id, current[3])
        )
